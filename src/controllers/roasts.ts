...
export async function updateRoast(req: Request, res: Response) {
  const roastIdToBeUpdated = req.body.id;
  const roastToBeUpdated = await Roast.findOne({ _id: roastIdToBeUpdated }).lean();
  ...
  try {
    const updatedRoast = await Roast.updateOne(
      { _id: roastIdToBeUpdated }, cleanRoastData, { upsert: true }
    );
